...
exports.getAllexpenses = async (req, res) => {
  ...
  try {
    ...
    const expenses = await Expense.findAll({ where: { userId } },{transaction});
  ...
  try {
    ...
    const userExpenses = await signupModel.findAll({
      attributes: [
        ...
        [sequelize.fn('COALESCE', sequelize.fn('SUM', sequelize.col('dailyexpenses.amount')), 0), 'totalExpense'], // Calculate total expense, handle nulls with COALESCE
      ],
      include: [
        {
